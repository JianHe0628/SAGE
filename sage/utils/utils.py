...
def plot_similarity_heatmap(similarity_matrix, title, output_dir, labels=False, count=0):
    ...
    plt.xlabel('GT Text Embeddings', fontproperties=my_font) 
    plt.ylabel('Segments (Visual Embeddings)', fontproperties=my_font)
    plt.title(f'Similarity Matrix Heatmap for {title}', fontproperties=my_font)
    plot_path = Path(output_dir) / f'Similarity_Heatmap_{count}.png'
...
def gumbel_softmax_sample(logits, temperature=1.0):
    ...
    Arguments:
    logits -- input logits (unnormalized log probabilities) of shape (batch_size, num_classes)
    temperature -- temperature for the softmax function. Higher temperature smooths the distribution, lower temperature makes it sharper.

    Returns:
    y -- Gumbel-Softmax sample, a differentiable approximation of categorical samples, shape (batch_size, num_classes)
    """
    ...
    y = F.softmax(logits_with_noise, dim=-1)
